showconsole()
mydir="./"
open(mydir .. "FEMM-2D-PL-HMSC-HMTW.FEM")
mi_saveas(mydir .. "FEMM-2D-PL-HMSC-HMTW_temp.fem")
...
NPRI=10--primary number of turns, initially here is physical (real)
NSEC1=20--secondary1 number of turns, initially here is physical (real)
NSEC2=20--secondary2 number of turns, initially here is physical (real)
K1=0.5
K2=0.5
ISEC1=-IPRI*NPRI/NSEC1*K1
ISEC2=-IPRI*NPRI/NSEC2*K2

PLANAR=1--if planar problem then 1, for axissymmetric problem should be 0
ALOT1=46.8--[mm] average length of turn (from datasheet) for PRIMARY winding 
ALOT2=38.8--[mm] average length of turn (from datasheet) for SEC1 winding 
ALOT3=54.8--[mm] average length of turn (from datasheet) for SEC2 winding 
Depth=7.85--[mm] parameter "Depth" of planar problem

--set problem parameters
--mi probdef(frequency,units,type,precision,(depth),(minangle),(acsolver))
if PLANAR==0 then
      	mi_probdef(F)
else 
	mi_probdef(F,"millimeters","planar",1e-8,Depth)
end

--calculation of NSEC1:NSEC2 turn ratio / First Simulation
...
--calculation of NPRI:NSEC2 turn ratio / Second Simulation
...
--calculation leak stored energy, primary and secondary1 and secondary2 are driven with currenst IPRI and ISEC1 and ISEC2  / Third Simulation
...
Rsec1=re(V2/I2)
Rsec2=re(V3/I3)
Lleak1_alt=im(V1/I1)/(2*pi*F)
Lleak2_alt=im(V2/I2)/(2*pi*F)
Lleak3_alt=im(V3/I3)/(2*pi*F)

--calculation primary stored energy when secondary1 and secondary2 are open  / Forth Simulation, actally can be combined with First Simulation
...
--calculation secondary1 stored energy when primary and secondary2 are open  / Fifth Simulation
...
--calculation secondary2 stored energy when primary and secondary1 are open  / Sixth Simulation
...
--Inductance calculation for the T-model (Lleak1, Lleak2, Lleak3,Lm, NPRI:NSEC1:NSEC2 with leakage inductances on primary and secondaries and calculated turn ratios)
...
--correctiom for planar problem
if PLANAR>0 then 
	Lleak1_alt=Lleak1_alt*ALOT1/(2*Depth)
	Lleak2_alt=Lleak2_alt*ALOT2/(2*Depth)
	Lleak3_alt=Lleak3_alt*ALOT3/(2*Depth)
	Lleak1=Lleak1*ALOT1/(2*Depth)
	Lleak2=Lleak2*ALOT2/(2*Depth)
	Lleak3=Lleak3*ALOT3/(2*Depth)
        Rpri=Rpri*ALOT1/(2*Depth)
	Rsec1=Rsec1*ALOT2/(2*Depth)
	Rsec2=Rsec2*ALOT3/(2*Depth)
end
...
print('Lleak1_alt=',Lleak1_alt)
print('Lleak2_alt=',Lleak2_alt)
print('Lleak3_alt=',Lleak3_alt)
print('Lleak1=',Lleak1)
print('Lleak2=',Lleak2)
print('Lleak3=',Lleak3)
print('Lm=',Lm)
...
--model file writing
model1_file=openfile("FEMM-2D-PL-HMSC-HMTW_three_winding_transformer_from_femm_t_model.lib","w")
write(model1_file,'.subckt FEMM-2D-PL-HMSC-HMTW_three_winding_transformer_from_femm_t_model 1 2 3 4 5 6\n')
...
write(model1_file,'.end FEMM-2D-PL-HMSC-HMTW_three_winding_transformer_from_femm_t_model \n')
